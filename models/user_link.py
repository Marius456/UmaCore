...
class UserLink:
    ...
    async def create(cls, discord_user_id: int, member_id: UUID, 
                     notify_on_deficit: bool = False) -> 'UserLink':
        """Create a new user link"""
        query = """
            INSERT INTO user_links (discord_user_id, member_id, notify_on_deficit)
            VALUES ($1, $2, $3)
            ...
            DO UPDATE SET 
                ...
                notify_on_deficit = $3,
                updated_at = NOW()
            RETURNING discord_user_id, member_id, notify_on_deficit, 
                      created_at, updated_at
        """
        row = await db.fetchrow(query, discord_user_id, member_id, notify_on_deficit)
    ...
    async def get_by_discord_id(cls, discord_user_id: int) -> Optional['UserLink']:
        ...
        query = """
            SELECT discord_user_id, member_id, notify_on_deficit, created_at, updated_at
            FROM user_links
    ...
    async def update_notifications(self, notify_on_deficit: bool):
        """Update notification preferences"""
        query = """
            UPDATE user_links
            SET notify_on_deficit = $1, updated_at = NOW()
